...
//
// Use w700_ucode_t from w700_ucode.h to decode from a uint64_t.
...
static void sysprinter(w700_sys_t *sys, int col, int drum) {
	char *s;
	int c;
...
static void sysdisplay(w700_sys_t *sys, int on) {
	if (!on) {
		// fputc('\b', stdout);
		// fputc(' ', stdout);
		// fflush(stdout);
...
static void syskeyboard(w700_sys_t *sys, uint16_t *kc, int ack) {
	if (kc == NULL) {
		// what to do?
		return;
	}
...
static uint8_t systape(w700_sys_t *sys, int wr, uint8_t nibble) {
...
static void syscn24(w700_sys_t *sys, uint8_t c, uint8_t sts) {
	if (sts != 1) {
...
static int intr(w700_sys_t *sys, int sig) {
...
static void dump(w700_sys_t *sys) {
	char buf[1024];
	fprintf(stderr, "at cycle %lld\n", sys->cpu.cycles);

	// todo: call w700_sim_cmd.c:_dump()...
...
static void sysfault(w700_sys_t *sys, const char *str) {
	fprintf(stderr, "%s\n", str);
	dump(sys);
	exit(1);
}

void sys_init(w700_sys_t *sys) {
	memset(pr_buf, ' ', sizeof(pr_buf));
...
void sys_start(w700_sys_t *sys, int ops) {
	_sys_ops = ops;
	...
	if (ops & SYS_BACK_END) {
		...
		printf("Wang 700 Programmable Calculator\n");
	}
}

void sys_stop(w700_sys_t *sys, int ops) {
	stop_fe(sys);
}

static int sys_interact(w700_sys_t *sys) {
...
void sys_setcass(w700_sys_t *sys, char *cass) {
	_cass_file = cass;
}
...
void sys_loadpgm(w700_sys_t *sys, char *pgm) {
	int fd;
	uint8_t *buf;
	struct stat stb;
...
static int run_some(w700_sys_t *sys, uint16_t entry) {
	int rc;
	sys->cpu.pc = entry;
	do {
		while (!sys->run) {
...
int sys_go(w700_sys_t *sys, uint16_t entry) {
	entry &= 0x0fff;
