...
#ident "$Id: w700_gui.c,v 1.1 2011/10/20 17:18:07 drmiller Exp $"
...
static void guidevinput(w700_sys_t *sys, uint16_t *kc, uint16_t b);

static void guikeyboard(w700_sys_t *sys, uint16_t *kc, int ack) {
	uint16_t b;
...
static void guiprinter(w700_sys_t *sys, int col, int drum) {
	uint16_t b;
	int rc;
...
static uint8_t guitape(w700_sys_t *sys, int wr, uint8_t nibble) {
	static uint8_t byte;
	static int bc = 0;
	uint16_t b;
	int rc;
...
static void guidev(w700_sys_t *sys, uint8_t c, uint8_t sts) {
	uint16_t b;
	int rc;
...
static int spawn_fe(w700_sys_t *sys) {
	int fd[2];
	int fe[2];
	...
	if (__gui_pid == 0) {
		...
		setsid();
#if 0
		execlp("xterm", "xterm", "-e", "./w700_fe", fdn, (char *)NULL);
		perror("xterm -e ./w700_fe");
#else
		execlp("java", "java", "w700_fe", "-b", (char *)NULL);
		perror("java w700_fe");
#endif
		exit(1);
	}
...
int start_fe(w700_sys_t *sys, int ops) {
	extraneous = 0;
	int rc = spawn_fe(sys);
	return rc;
}

void stop_fe(w700_sys_t *sys) {
	if (__gui_pid > 0) {
		close(__gui_kfd);
...
void setup_fe(w700_sys_t *sys, int ops) {
	if (ops & SYS_BACK_END) {
		sys->keyboard = guikeyboard;
		sys->display = guidisplay;
		sys->printer = guiprinter;
