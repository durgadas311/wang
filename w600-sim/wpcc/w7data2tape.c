...
uint8_t data[256][16]; // there will never be more that MAX_REG registers...
...
int main(int argc, char **argv) {
	...
	x = 0;
	while (x < MAX_REG && fgets(buf, sizeof(buf), fp) != NULL) {
		double d;
		++__line__;
		c = sscanf(buf, " %lf\n", &d);
		if (c == 1) {
			if (w7_do_data_d(d, data[x]) != 0) {
				exit(1);
			}
		} else {
			// get description?
